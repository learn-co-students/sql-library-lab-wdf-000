...
def select_series_title_with_most_human_characters
 "SELECT series.title from characters JOIN series ON characters.series_id = series.id WHERE characters.species = 'human' GROUP BY series.title ORDER BY COUNT(characters.id) DESC LIMIT 1;"
end
