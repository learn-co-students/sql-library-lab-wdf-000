...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM authors INNER JOIN series ON authors.id = series.author_id INNER JOIN characters ON authors.id = characters.author_id WHERE characters.species = 'human' GROUP BY series.title LIMIT 1;"
end
